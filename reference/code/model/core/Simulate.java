    ...
    private void createMatchResultsAndBonus(Match match) {
        ...
        for (Player player : match.getTeamLocal().getTactic().getPlayers()) {
            if (player.getIsInLineup().booleanValue()) {
                ...
                localTeamAllAverage = (int) (localTeamAllAverage + (player.getAverageSkillAllByPosition(player.getPositionMatch()).doubleValue() * (player.getIsTitular().booleanValue() ? 2 : 1)));
                numOfPlayers++;
            }
        }
        ...
        for (Player player2 : match.getTeamVisitor().getTactic().getPlayers()) {
            if (player2.getIsInLineup().booleanValue()) {
                ...
                visitorTeamAllAverage = (int) (visitorTeamAllAverage + (player2.getAverageSkillAllByPosition(player2.getPositionMatch()).doubleValue() * (player2.getIsTitular().booleanValue() ? 2 : 1)));
                numOfPlayers2++;
            }
        }
