...
class GameViewModel(private val database: AppDatabase) : ViewModel() {
    ...
    val draftPlayers: StateFlow<List<re.manager.basket.data.entity.PlayerEntity>> = _currentGameId.flatMapLatest { gameId ->
        if (gameId != null) {
            database.playerDao().getRookiesFlow(gameId).map { players ->
                players.sortedByDescending { it.potential }
            }
        } else {
            flowOf(emptyList())
        }
                ...
                if (satisfaction >= 80) {
                    ...
                    if (bestOffer.teamId == userTeamId) {
                        database.newsDao().insert(re.manager.basket.data.entity.NewsEntity(
                            ...
                            body = "Success: ${player.name} has accepted your contract offer of $${bestOffer.salary}!"
                        ))
                        ...
                        database.newsDao().insert(re.manager.basket.data.entity.NewsEntity(
                            ...
                            body = "Failed: ${player.name} has rejected your offer and signed with another team."
                        ))
                    }
                } else if (offers.any { it.teamId == userTeamId }) {
                    // All offers rejected
                    database.newsDao().insert(re.manager.basket.data.entity.NewsEntity(
                        ...
                        type = "SIGNING_FAILED",
                        title = "Contract Rejected",
                        body = "Rejected: ${player.name} found your offer insufficient."
                    ))
                }
            }
    ...
    private suspend fun startNewSeason(gameId: Int, oldSeason: Int) {
        ...
        val allPlayers = database.playerDao().getPlayersByGame(gameId)
        val evolver = re.manager.basket.domain.engine.PlayerDevelopmentEngine()
