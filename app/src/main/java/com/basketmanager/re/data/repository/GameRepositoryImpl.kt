package com.basketmanager.re.data.repository

import android.content.ContentValues
import android.content.Context
import android.database.Cursor
import com.basketmanager.re.data.local.database.BasketManagerDatabaseHelper
import com.basketmanager.re.data.local.database.BasketManagerDatabaseHelper.Companion as DB
import com.basketmanager.re.data.local.database.toDomain
import com.basketmanager.re.data.local.database.toEntity
...
class GameRepositoryImpl(private val context: Context) : GameRepository {
    ...
    override suspend fun updateGame(game: GameSession) = withContext(Dispatchers.IO) {
        val db = dbHelper.writableDatabase
    ...
    override suspend fun updateTeam(team: Team) = withContext(Dispatchers.IO) {
        val db = dbHelper.writableDatabase
        val cv = ContentValues().apply {
    ...
    override suspend fun updatePlayers(players: List<Player>) = withContext(Dispatchers.IO) {
        val db = dbHelper.writableDatabase
        db.beginTransaction()
        try {
            players.forEach { p ->
    ...
    override suspend fun updateTactic(tactic: Tactic) = withContext(Dispatchers.IO) {
        val db = dbHelper.writableDatabase
    ...
    override suspend fun addNews(newsItem: NewsItem) = withContext(Dispatchers.IO) {
        val db = dbHelper.writableDatabase
