...
public class Player extends BlankBase {
    ...
    public Double getValue() {
        return Double.valueOf((getAverageSkillAll().doubleValue() + ((double) (getPotential().intValue() * 2))) - ((double) (getAge().intValue() / 2)));
    }

    public Double getMatchValue(Integer position) {
        return Double.valueOf(((getAverageSkillAll().doubleValue() + (((double) getPotential().intValue()) * 1.8d)) - ((double) (getAge().intValue() / Util.getRandomValue(2, 3).intValue()))) + ((double) getPenalty(position).intValue()));
    }

    public double getMarketValue() {
        double marketValue = ((((getValue().doubleValue() - 70.0d) * Math.abs(getValue().doubleValue() - 70.0d)) / 4.0d) - ((double) ((getSalary().intValue() / 2500000) + (((getAge().intValue() - 18) * (getAge().intValue() - 18)) / 15)))) + 10.0d;
        if (getAge().intValue() < 24 && getPotential().intValue() > 8) {
            marketValue += (double) getPotential().intValue();
        }
        if (getAverageSkillAll().doubleValue() > 79.0d) {
            marketValue += (double) (getPotential().intValue() + 10);
        }
        if (marketValue < 0.0d) {
    ...
    private Double getAverageForPosition(Integer position, boolean attack, boolean defense) {
        ...
        if (position != null && position.intValue() != 0) {
            if (attack) {
                average = Double.valueOf(average.doubleValue() + ((double) (((((getSkillPass().intValue() * getBaseOfPosition(position.intValue(), 5).intValue()) + (getSkillShotInterior().intValue() * getBaseOfPosition(position.intValue(), 6).intValue())) + (getSkillShotExterior().intValue() * getBaseOfPosition(position.intValue(), 7).intValue())) + (getSkillShotFree().intValue() * getBaseOfPosition(position.intValue(), 8).intValue())) / getAttackDivisor(position.intValue()).intValue())));
            }
            if (defense) {
                average = Double.valueOf(average.doubleValue() + ((double) (((((getSkillPhysique().intValue() * getBaseOfPosition(position.intValue(), 1).intValue()) + (getSkillBlock().intValue() * getBaseOfPosition(position.intValue(), 2).intValue())) + (getSkillSteal().intValue() * getBaseOfPosition(position.intValue(), 3).intValue())) + (getSkillRebound().intValue() * getBaseOfPosition(position.intValue(), 4).intValue())) / getDefenseDivisor(position.intValue()).intValue())));
            }
