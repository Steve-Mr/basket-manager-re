...
public class Team extends BlankBase {
    ...
    public Double getTeamAverageAge() {
        ...
        for (Player player : getTactic().getPlayers()) {
            if (i < 3) {
                average += (double) (player.getAge().intValue() * 10);
                divisor += 10.0d;
            } else if (i < 6) {
                average += (double) (player.getAge().intValue() * 5);
                divisor += 5.0d;
            } else if (i < 10) {
                average += (double) (player.getAge().intValue() * 2);
                divisor += 2.0d;
            } else {
                average += (double) (player.getAge().intValue() * 0);
                divisor += 0.0d;
            }
            i++;
        }
    ...
    public Double getTeamAveragePotential() {
        ...
        for (Player player : getTactic().getPlayers()) {
            if (i < 3) {
                average += (double) (player.getPotential().intValue() * 10);
                divisor += 10.0d;
            } else if (i < 6) {
                average += (double) (player.getPotential().intValue() * 5);
                divisor += 5.0d;
            } else if (i < 10) {
                average += (double) (player.getPotential().intValue() * 2);
                divisor += 2.0d;
            }
            i++;
        }
    ...
    private Double getPlayersAverageSkillsAll(List<Player> players, Boolean isAttack) {
        ...
        for (Player player : players) {
            if (isAttack == null) {
                dDoubleValue = player.getAverageSkillAll().doubleValue();
            } else if (isAttack.booleanValue()) {
                dDoubleValue = player.getAverageSkillAttack().doubleValue();
            } else {
                dDoubleValue = player.getAverageSkillDefense().doubleValue();
            }
            average += dDoubleValue;
            i++;
        }
        if (i == 0) {
            return Double.valueOf(0.0d);
        }
        return Double.valueOf(average / ((double) i));
    }
}
