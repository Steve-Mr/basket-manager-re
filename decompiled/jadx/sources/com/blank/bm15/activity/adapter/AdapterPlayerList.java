...
public class AdapterPlayerList extends ArrayAdapter<Player> {
    private Context context;
    private int layoutId;
    private List<Player> playerList;
    private Boolean showTeamName;

    public AdapterPlayerList(Context context, int layoutId, List<Player> playerList, boolean showTeamName) {
        super(context, layoutId, playerList);
        this.playerList = null;
        this.context = context;
        this.layoutId = layoutId;
        this.showTeamName = Boolean.valueOf(showTeamName);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (player.getStateInjury().intValue() > 0) {
            ...
            if (!this.showTeamName.booleanValue() && posTitular != 0) {
                if (posTitular == player.getPositionFirst().intValue()) {
                    holder.textViewPositionFirst.setTextColor(this.context.getResources().getColor(R.color.holo_blue_dark));
                } else if (posTitular == player.getPositionSecond().intValue()) {
                    holder.textViewPositionSecond.setTextColor(this.context.getResources().getColor(R.color.holo_blue_dark));
                } else {
                    holder.textViewSeparatorPositionMatch.setVisibility(0);
                    holder.textViewPositionMatch.setVisibility(0);
                    holder.textViewPositionMatch.setText(Util.getLongPositionString((Activity) this.context, Integer.valueOf(posTitular)));
                }
            ...
            } else if (!this.showTeamName.booleanValue() && posReserve != 0) {
                if (posReserve == player.getPositionFirst().intValue()) {
                    holder.textViewPositionFirst.setTextColor(this.context.getResources().getColor(R.color.holo_green_dark));
                } else if (posReserve == player.getPositionSecond().intValue()) {
                    holder.textViewPositionSecond.setTextColor(this.context.getResources().getColor(R.color.holo_green_dark));
                } else {
                    holder.textViewSeparatorPositionMatch.setVisibility(0);
                    holder.textViewPositionMatch.setVisibility(0);
                    holder.textViewPositionMatch.setText(Util.getLongPositionString((Activity) this.context, Integer.valueOf(posReserve)));
                }
        ...
        String name = "";
        if (this.showTeamName.booleanValue() && player.getTeam().getId() != null) {
            name = String.valueOf("") + "(" + player.getTeam().getName() + ") ";
        }
        holder.textViewName.setText(String.valueOf(name) + player.getShortName());
        if (this.showTeamName.booleanValue()) {
            ...
            holder.textViewAgeText.setText(this.context.getString(R.string.spinner_player_age));
            holder.textViewAgeValue.setText(BlankObj.toString(player.getAge()));
            holder.textViewFormText.setText(this.context.getString(R.string.spinner_player_form));
            holder.textViewFormValue.setText(BlankObj.toString(player.getStateForm()));
            holder.textViewEnergyText.setText(this.context.getString(R.string.spinner_player_energy));
            holder.textViewEnergyValue.setText(BlankObj.toString(player.getStateEnergy()));
        }
